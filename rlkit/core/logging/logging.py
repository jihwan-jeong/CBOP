...
from rlkit.core.logging.tabulate import tabulate
...
class Logger(object):
    ...
    def save_itr_params(self, epoch, state_dict, prefix='', extension='pth'):
        state_dict = state_dict.copy()
        # for k in list(state_dict.keys()):        
        #     if k.split('/')[0] == 'evaluation':
        #         del state_dict[k]
        if self._snapshot_dir:
            prefix = prefix.replace(' ', '_')
            if self._snapshot_mode == 'all':
                file_name = osp.join(self._snapshot_dir, f'{prefix}_epoch_{epoch}.{extension}')
                torch.save(state_dict, file_name)
            elif self._snapshot_mode == 'last':
                # override previous params
                file_name = osp.join(self._snapshot_dir, f'{prefix}_checkpoint.{extension}')
                torch.save(state_dict, file_name)
            elif self._snapshot_mode == "gap":
                if epoch % self._snapshot_gap == 0:
                    file_name = osp.join(self._snapshot_dir, f'{prefix}_epoch_{epoch}.{extension}')
                    torch.save(state_dict, file_name)
                ...
                file_name = osp.join(self._snapshot_dir, f'{prefix}_checkpoint.{extension}')
                torch.save(state_dict, file_name)
            elif self._snapshot_mode is None:
                pass
            else:
                raise NotImplementedError
        else:
            raise AttributeError("Logger._snapshot_dir not set!")
